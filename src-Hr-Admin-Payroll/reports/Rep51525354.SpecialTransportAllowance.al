report 51525354 "Special Transport Allowance"
{
    ApplicationArea = All;
    Caption = 'Special Transport Allowance Report';
    UsageCategory = ReportsAndAnalysis;
    DefaultLayout = RDLC;
    RDLCLayout = './src/reports/layouts/SpecialTransportAllowanceReport.rdlc';
    dataset
    {
        dataitem(Employee; Employee)
        {
            ...
            column(StatutoryNetAmount; StatutoryNetAmount)
            {
            }
            column(Paye; Paye)
            {
            }
            column(SpecialTransportAllowance; SpecialTransportAllowance)
            {
            }
            column(SpecialTransportAllowanceArrears; SpecialTransportAllowanceArrears)
            {
            }
            ...
            begin
                ...
                if not ComapnyInfoCaptured then begin
                    ...
                    ReportTitle := 'TRANSPORT ALLOWANCE FOR ' + UpperCase(Format(PayrollPeriod, 0, '<Month Text> <Year4>'));
                ...
                Paye := 0;
                StatutoryNetAmount := 0;
                SpecialTransportAllowance := 0;
                SpecialTransportAllowanceArrears := 0;
                Department := '';
                Section := '';
                Designation := '';
                ...
                AssignmentMatrix.SetRange("Special Transport Allowance", true);
                if SelectedPayrollCountry <> '' then
                ...
                if AssignmentMatrix.FindSet() then
                    repeat
                        ...
                        if AssignmentMatrix."Transaction Title" = 'SPECIAL TRANSPORT ALLOWANCE ARREAS' then begin
                            SpecialTransportAllowanceArrears := abs(AssignmentMatrix.Amount) * ExchangeRate;
                            //StatutoryNetAmount := abs(AssignmentMatrix."Statutory Net") * ExchangeRate;
                        end else begin
                            SpecialTransportAllowance := abs(AssignmentMatrix.Amount) * ExchangeRate;
                            StatutoryNetAmount := abs(AssignmentMatrix."Statutory Net") * ExchangeRate;
                        end;
                    until AssignmentMatrix.Next() = 0;
                //end;

                if (SpecialTransportAllowance = 0) and (SpecialTransportAllowanceArrears = 0) then
                    CurrReport.Skip()
                else begin
                    //Get akina department
                    PeriodMovement.Reset();
                    PeriodMovement.SetRange("Emp No.", Employee."No.");
                    PeriodMovement.SetRange("Payroll Period", PayrollPeriod);
                    if PeriodMovement.FindFirst() then begin
                        Department := PeriodMovement."Department Name";
                        Section := PeriodMovement."Section Title";
                        Designation := PeriodMovement."Job Title";
                    end;
                end;
    ...
    var
        ...
        ReportTitle: Text;
        Paye: Decimal;
        StatutoryNetAmount: Decimal;
        SpecialTransportAllowance: Decimal;
        SpecialTransportAllowanceArrears: Decimal;
