report 51525326 "New PAYE Annexture"
{
    ApplicationArea = All;
    Caption = 'New PAYE Annexture';
    UsageCategory = ReportsAndAnalysis;
    RDLCLayout = './src/reports/layouts/NewPAYEAnnexture.rdlc';
    dataset
    {
        dataitem(Employee; Employee)
        {
            ...
            column(LumpSumTransport; LumpSumTransport)
            { }
            column(TerminalBenefits; TerminalBenefits)
            {
            }
            column(RetirementBenefits; RetirementBenefits)
            {
            }
            ...
            { }
            column(IsEmpRama; IsEmpRama)
            { }
            column(PaysCBHI; PaysCBHI)
            { }
            column(EmployeeCategory; EmployeeCategory)
            { }
            ...
            begin
                ...
                PaysCBHI := 'N';
                EmployeeCategory := 'P';
                if Employee."Payroll Country" in ['CABIN'] then
                    EmployeeCategory := 'S';
                if Employee."Payroll Country" in ['RWANDA', 'EXPATRIATE', 'CABIN'] then
                    PaysCBHI := 'Y';
                if ("National ID" = '') and ("ID Number" <> '') then
                ...
                if AssignmentMatrix.FindSet() then
                    repeat
                        ...
                        if (not AssignmentMatrix."Basic Salary Code" and not AssignmentMatrix."Transport Allowance" and not AssignmentMatrix."Housing Allowance"  /*NOT (AssignmentMatrix.Code IN ['R3', 'E02', 'E2', 'R2', 'E01', 'E1', 'R4', 'E04'])*/) and (AssignmentMatrix.Type = AssignmentMatrix.Type::Payment) and (AssignmentMatrix."Is Flat Amount" = true) and (not (AssignmentMatrix."Transaction Title" in ['LUMPSUM ALLOWANCE', 'OVERTIME ALLOWANCE'/*, 'IN-HOUSE INSTRUCTOR ALLOWANCE'*/])) then //Other cash allowance - any other flat entry - we'll let 'IN-HOUSE INSTRUCTOR ALLOWANCE' go here
                        ...
                        /*if AssignmentMatrix."Transaction Title" = 'OVERTIME ALLOWANCE' then
                            OtherCashAllowance += TransAmount;*/
                        if AssignmentMatrix."Transaction Title" = 'LUMPSUM ALLOWANCE' then
                            LumpSumTransport += TransAmount;
                        TransAmount := 0;
                    until AssignmentMatrix.Next() = 0;


                //Get overtime for the specific month
                AssignmentMatrix.Reset();
                AssignmentMatrix.SetRange("Transaction Title", 'OVERTIME ALLOWANCE');
                AssignmentMatrix.SetRange("Employee No", Employee."No.");
                //Starting March 2025, we get overtime per period processed, not overtime period
                if PayrollPeriod >= 20250301D then
                    AssignmentMatrix.SetRange("Payroll Period", PayrollPeriod)
                else
                    AssignmentMatrix.SetRange("Overtime Period", PayrollPeriod);
                if SelectedPayrollCountry <> '' then
                    AssignmentMatrix.SetFilter(Country, SelectedPayrollCountry);
                if AssignmentMatrix.FindSet() then
                    repeat
                ...
                TerminalDuesHeader.SetRange("WB No.", Employee."No.");
                TerminalDuesHeader.SetRange("Period Processed", PayrollPeriod);
                if SelectedPayrollCountry <> '' then
                    TerminalDuesHeader.SetRange("Payroll Country", SelectedPayrollCountry);
                TerminalDuesHeader.SetRange("Approval Status", TerminalDuesHeader."Approval Status"::Released);
                if TerminalDuesHeader.FindSet() then
                    repeat
                        TerminalDuesLines.Reset();
                        TerminalDuesLines.SetRange("Header No.", TerminalDuesHeader."No.");
                        TerminalDuesLines.SetRange("Trans Type", TerminalDuesLines."Trans Type"::Earning); //Seems we're only taking earnings
                        if TerminalDuesLines.FindSet() then
                            repeat
                                TransAmount := Round(GetInDesiredCurrency(TerminalDuesHeader."Payroll Currency", TerminalDuesHeader."Payroll Country", ABS(TerminalDuesLines.Amount)));
                                if TerminalDuesLines."Is Basic" then
                                    BasicSalary += TransAmount
                                else if TerminalDuesLines."Is House" then
                                    CashAllowanceHouse += TransAmount
                                else if TerminalDuesLines."Is Transport" then
                                    CashAllowanceTransport += TransAmount
                                else
                                    OtherCashAllowance += TransAmount;
                            until TerminalDuesLines.Next() = 0;

                        //Reduce less days deductions from othercash allowance
                        AdditionalEntitledTos.Reset();
                        AdditionalEntitledTos.SetRange("Header No.", TerminalDuesHeader."No.");
                        AdditionalEntitledTos.SetRange(Action, AdditionalEntitledTos.Action::Less);
                        if AdditionalEntitledTos.FindSet() then
                            repeat
                                TerminalDuesLines.Reset();
                                TerminalDuesLines.SetRange("Header No.", TerminalDuesHeader."No.");
                                TerminalDuesLines.SetRange("Trans Type", TerminalDuesLines."Trans Type"::Deduction);
                                TerminalDuesLines.SetRange(Description, Format(AdditionalEntitledTos."No. of Days") + ' days'' ' + AdditionalEntitledTos.Description);
                                if TerminalDuesLines.FindSet() then
                                    repeat
                                        TransAmount := Round(GetInDesiredCurrency(TerminalDuesHeader."Payroll Currency", TerminalDuesHeader."Payroll Country", ABS(TerminalDuesLines.Amount)));
                                        OtherCashAllowance += -Abs(TransAmount);
                                    until TerminalDuesLines.Next() = 0;
                            until AdditionalEntitledTos.Next() = 0;

                        TerminalDuesHeader.CalcFields(Balance);
                        TransAmount := Round(GetInDesiredCurrency(TerminalDuesHeader."Payroll Currency", TerminalDuesHeader."Payroll Country", ABS(TerminalDuesHeader.Balance)));
                        /*if TerminalDuesHeader.Type = TerminalDuesHeader.Type::"Final Dues" then
                            TerminalBenefits += TransAmount;*/
                        if TerminalDuesHeader.Type = TerminalDuesHeader.Type::"Retirement Benefits" then
                            RetirementBenefits += TransAmount;
                    until TerminalDuesHeader.Next() = 0;

                if (BasicSalary <> 0) or (CashAllowanceHouse <> 0) or (CashAllowanceTransport <> 0) or (OtherCashAllowance <> 0) then
                    SkipEmployee := false;
                if (RetirementBenefits <> 0) or (TerminalBenefits <> 0) then
                    SkipEmployee := false;

                if (EmployeeNSSFDeduction = 0) AND (EmployeeRAMADeduction = 0) and (BasicSalary = 0) and (CashAllowanceHouse = 0) and (CashAllowanceTransport = 0) and (OtherCashAllowance = 0) then
                    SkipEmployee := true;
                if IsRAMA then begin
