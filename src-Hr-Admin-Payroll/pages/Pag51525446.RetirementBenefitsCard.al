page 51525446 "Retirement Benefits Card"
{
    ApplicationArea = All;
    Caption = 'Retirement Benefits Card';
    PageType = Card;
    SourceTable = "Terminal Dues Header";
    PromotedActionCategories = 'New,Process,Report,Approval Request';
    SourceTableView = WHERE(Type = const("Retirement Benefits"));
    ...
    layout
    {
        area(Content)
        {
            group(General)
            {
                ...
                field("Years in Service"; Rec."Years in Service")
                { }
                field("Gross Salary Multiplier"; Rec."Gross Salary Multiplier")
                { }
                ...
                begin
                    ...
                    if periodrec.FindLast then begin
                        ...
                        if EmpRec.FindFirst() then begin
                            ...
                            EmpRec."Retirement Benefits Comp." := true;
                            EmpRec.Modify();
                        end;
                        ...
                        if AssingmentMatrix.FindSet() then
                            repeat
                                ...
                                /*if Rec."Payroll Country" = 'BURUNDI' then begin
                                    Message('%1 Earning Currency: %2 Amount FCY: %3 | Amount: %4', SalaryStructure.Description, AssingmentMatrix."Earning Currency", AssingmentMatrix."Amount In FCY", AssingmentMatrix.Amount);
                                end;*/
                        ...
                        if SalaryStructure.FindSet() then
                            repeat
                                ...
                                Descr := Format(Rec."Unpaid Worked Days") + ' days'' ' + SalaryStructure.Description + ' for ' + Rec."Final Month of Service";
                        ...
                        if Rec."Untaken Leave Days" <> 0 then begin
                            ...
                            Descr := Format(Rec."Untaken Leave Days") + ' days of untaken leave';
                        ...
                        if (Rec."Gross Salary" <> 0) /*and (Rec."Taxable Final Dues" > 0)*/ then begin
                            ...
                            Countries.Reset();
                            Countries.SetRange(Code, Rec."Payroll Country");
                            if Countries.FindFirst() then begin
                                if Countries."Retirement Benefit Table" = '' then
                                    Error('Kindly set the Retirement Benefits Table for country %1', Rec."Payroll Country");
                                Rec.ComputeYearsInService();
                                Rec.Period := Rec.getDuration();
                                DecimalMultiplier := abs(GetPaye.GetNonTaxTableDeduction(Rec."Years in Service", Rec."Payroll Country", '', Countries."Retirement Benefit Table", 0));
                                Rec."Gross Salary Multiplier" := DecimalMultiplier; //We hope it'll convert
                                if Rec."Gross Salary Multiplier" <> 0 then begin
                                    LineNo += 1;
                                    WorkingAmount := Rec."Gross Salary" * Rec."Gross Salary Multiplier";
                                    RetirementBenefits := WorkingAmount;
                                    Descr := 'Retirement Benefits (' + Format(Rec."Gross Salary Multiplier") + ' times Gross Salary)';
                                    InsertTerminalDueLine(LineNo, true, true, Rec."Payroll Currency", Descr, WorkingAmount, false, false, false);
                                end;
                            ...
                            if Rec."Payroll Country" in ['RWANDA', 'CABIN', 'EXPATRIATE'] then begin
                                ...
                                SocialSecurity := (Rec."Taxable Final Dues" - RetirementBenefits) * (6 / 100);
                                ...
                                LineNo += 1;
                                //OccupationalHazard := (Rec."Taxable Final Dues" - RetirementBenefits - Rec."Total Transport") * (2 / 100);                                
                        ...
                        if EmpRec.FindFirst() then begin
                            ...
                            EmpRec."Retirement Benefits Comp." := false;
                            EmpRec.Modify();
                        end;
    ...
    begin
        Rec.Type := Rec.Type::"Retirement Benefits";
    end;
