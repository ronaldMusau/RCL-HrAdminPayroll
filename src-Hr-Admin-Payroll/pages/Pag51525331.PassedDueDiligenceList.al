page 51525331 "Passed Due Diligence List"
{
    ApplicationArea = All;
    ...
    SourceTableView = WHERE("Passed Due Diligence" = CONST(true));
            ...
            action("Second Interview Invite")
            {
                ...
                begin
                    ...
                    if RecruitmentNeed.FindFirst() then begin
                        if RecruitmentNeed."Oral Interview Venue" = '' then
                        ...
                        if RecruitmentNeed."Current Stage" <> RecruitmentNeed."Current Stage"::"Second Interview" then
                            Error('You can only send oral interview invites when the current stage is Second Interview!');

                        if (RecruitmentNeed."Second Interview Inv Emailed") then begin
                            if not confirm('The oral interview invite was already sent on ' + format(RecruitmentNeed."Sec. Interview Inv Emailed On") + '. Do you still want to send?') then
                        ...
                        if JobApplicationsTable.FindFirst then begin
                            ...
                            repeat
                                ...
                                '<p>Following your successful shortlisting and initial screening for the ' + JobApplicationsTable.JobDescription + ' position, we are pleased to invite you for an oral interview as follows;</p>' +
                                '<p><ul><li>Interview Date & Time: ' + FORMAT(RecruitmentNeed."Oral Interview DateTime") + '</li>' +
                                '<li>Venue: ' + RecruitmentNeed."Oral Interview Venue" + '</li></ul> +</p>' +
                        ...
                        Window.close();
                        RecruitmentNeed."Second Interview Inv Emailed" := true;
                        RecruitmentNeed."Sec. Interview Inv Emailed On" := CREATEDATETIME(TODAY, TIME);
